...
fn last_day_in_month(year: i32, month: u32) -> NaiveDate {
    ...
    // 先生成一个NaiveDate，用的是当月第一天，然后pred返回前一天的Native Date
    NaiveDate::from_ymd(y, m, 1).pred()
}

fn format_month(year: i32, month: u32, print_year: bool, today: NaiveDate) -> Vec<String> {
    let first = NaiveDate::from_ymd(year, month, 1);
    // weekday().number_from_sunday，从sunday开数到现在的个数？
    ...
    // 使用each chrono::Datelike::day来form month
    ...
    lines.push("Su Mo Tu We Th Fr Sa  ".to_string()); // 俩空格;

    // days里按照7个string一组，分开
